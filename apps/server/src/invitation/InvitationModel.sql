CREATE TABLE IF NOT EXISTS invitations (
  ...
  status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'accepted', 'rejected', 'expires')),
...
CREATE INDEX IF NOT EXISTS idx_invitations_expired_at ON invitations(expired_at, status) WHERE status = 'pending';
